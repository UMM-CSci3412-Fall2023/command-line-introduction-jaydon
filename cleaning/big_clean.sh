...
tempdir=$(mktemp -dt tmp.XXXXXXXX)
...
for file in *; do
	...
	# The -n flag checks that the variable is empty; i.e., if grep does contain some output, delete the file
	if [ -n "${out}" ]; then
		rm "${file}"
	fi
done
